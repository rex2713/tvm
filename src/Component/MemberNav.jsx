import React from "react";
import { Link } from "react-router-dom";

const MemberNav = () => {
  return (
    <section className="flex h-[240px] w-full justify-between bg-gradient-to-b from-[#082A4D] via-[#041526] to-[#000000] px-40 pt-5">
      <h2 className="align-bottom font-['Ubuntu'] text-[140px] font-bold leading-[300px] text-white [text-shadow:4px_4px_#A8D916]">
        Member
      </h2>
      <div className="mb-4 flex items-end gap-8  whitespace-nowrap ">
        <Link
          to="/member/login"
          className="transform-style-3d relative rounded-full  border-2  border-solid  border-[#595959] bg-[#040F26] px-6 py-2 text-xl text-white/70 transition-all duration-[400ms]  before:absolute before:bottom-0 before:left-0  before:right-0 before:top-0 before:h-full before:w-full before:rounded-full before:bg-[#595959] before:transition-all before:duration-300 before:[transform:translate3d(0,8px,-1em)] hover:border-[#E8F8B9] hover:bg-[#BACC85] hover:text-[#FFFFFF] hover:[transform:translate3d(0em,-4px,0)] hover:before:bg-[#E8F8B9] hover:before:shadow-[0_6px_0_#E8F8B9] hover:before:[transform:translate3d(0em,6px,-1em)] active:bg-[#98C414] active:[transform:translate3d(0,4px,0)] active:before:bg-[#E8F8B9] active:before:shadow-[0_0_0_#E8F8B9] active:before:[transform:translate3d(0,4px,-1em)]"
        >
          會員登入
        </Link>
        <Link
          to="/member/sign"
          className="transform-style-3d relative rounded-full  border-2  border-solid  border-[#595959] bg-[#040F26] px-6 py-2 text-xl text-white/70 transition-all duration-[400ms]  before:absolute before:bottom-0 before:left-0  before:right-0 before:top-0 before:h-full before:w-full before:rounded-full before:bg-[#595959] before:transition-all before:duration-300 before:[transform:translate3d(0,8px,-1em)] hover:border-[#E8F8B9] hover:bg-[#BACC85] hover:text-[#FFFFFF] hover:[transform:translate3d(0em,-4px,0)] hover:before:bg-[#E8F8B9] hover:before:shadow-[0_6px_0_#E8F8B9] hover:before:[transform:translate3d(0em,6px,-1em)] active:bg-[#98C414] active:[transform:translate3d(0,4px,0)] active:before:bg-[#E8F8B9] active:before:shadow-[0_0_0_#E8F8B9] active:before:[transform:translate3d(0,4px,-1em)]"
        >
          註冊會員
        </Link>
      </div>
    </section>
  );
};

export default MemberNav;
